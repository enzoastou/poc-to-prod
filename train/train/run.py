...
def train(dataset_path, train_conf, model_path, add_timestamp):
    ...
    dataset = LocalTextCategorizationDataset(filename = dataset_path, batch_size = train_conf["batch_size"],
                                             train_ratio = train_conf["train_ratio"],
    ...
    with open(os.path.join(artefacts_path, "params.json"), "w") as fileStream:
        json.dump(train_conf, fileStream)

    # save labels index in artefacts_path/labels_index.json
    with open(os.path.join(artefacts_path, "labels_index.json"), "w") as fileStream:
        json.dump(dataset.get_label_to_index_map(), fileStream)
    ...
    with open(os.path.join(artefacts_path, "train_output.json"), "w") as fileStream:
        json.dump(serializable_hist, fileStream)
