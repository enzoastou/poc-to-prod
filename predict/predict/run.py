...
class TextPredictionModel:
    ...
    def from_artefacts(cls, artefacts_path: str):
        ...
        # load params
        paramsFile = open(os.path.join(artefacts_path, "params.json"))
        params = json.load(paramsFile)

        # load labels_to_index
        labelsToIndexFile = open(os.path.join(artefacts_path, "labels_index.json"))
        labels_to_index = json.load(labelsToIndexFile)
    ...
    def predict(self, text_list, top_k=5):
        ...
        tags_indexes = argsort(tag_pred)[0][-top_k:]
